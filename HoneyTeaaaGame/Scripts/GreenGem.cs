...
public partial class GreenGem : Gem
{
    ...
    public async override void Trigger()
    {
		...
		//Create the green power particles
		Vector2 pos = GlobalPosition + new Vector2(40, 40);
		Node2D powerFX = CreatePowerParticles(Colors.LimeGreen, pos, (Node2D)Battle.Instance, (damage).ToString());

		//Move green particles next to white particles
		...
		//Move the white mult particles to green power
		...
		//Move the cyan magic particles to green power particles
		...
		//Move the green power particles to player
		tween = GetTree().CreateTween().BindNode(this).SetTrans(Tween.TransitionType.Quint);
		tween.TweenProperty(powerFX, "global_position", Battle.Instance.player.GlobalPosition, 0.5f * Battle.Instance.speedMult);
		await ToSignal(tween, "finished");
		powerFX.QueueFree();
